...
declare global {
     ...
     * 
     *   Read How to label your p5.js code to learn more 
     *   about making sketches accessible.
     ...
     *   groups of shapes that create meaning together. The 
     *   first parameter, name, is the name of the element. 
     * 
    ...
    /**
     *   This makes the canvas forget how far from the 
     *   camera everything that has been drawn was. Use 
     *   this if you want to make sure the next thing you 
     *   draw will not draw behind anything that is already 
     *   on the canvas. This is useful for things like 
     *   feedback effects, where you want the previous 
     *   frame to act like a background for the next frame, 
     *   and not like a plane in 3D space in the scene. 
     * 
     *   This method is only available in WebGL mode. Since 
     *   2D mode does not have 3D depth, anything you draw 
     *   will always go on top of the previous content on 
     *   the canvas anyway.
     *   @param [depth] The value, between 0 and 1, to 
     *   reset the depth to, where 0 corresponds to a value 
     *   as close as possible to the camera before getting 
     *   clipped, and 1 corresponds to a value as far away 
     *   from the camera as possible. The default value is 
     *   1.
     */
     ...
     *   @chainable
     */
    function applyMatrix(a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number, k: number, l: number, m: number, n: number, o: number, p: number): p5
     ...
     *   matches the given CSS selector string. The string 
     *   can be an ID, class, tag name, or a combination. 
     *   select() returns a p5.Element object if it finds a 
     *   match and null otherwise. The second parameter, 
     *   container, is optional. It specifies a container 
     *   to search within. container can be CSS selector 
     *   string, a p5.Element object, or an HTMLElement 
     *   object.
     ...
     *   the given CSS selector string. The string can be 
     *   an ID, class, tag name, or a combination. 
     *   selectAll() returns an array of p5.Element objects 
     *   if it finds any matches and an empty array 
     *   otherwise. The second parameter, container, is 
     *   optional. It specifies a container to search 
     *   within. container can be CSS selector string, a 
     *   p5.Element object, or an HTMLElement object.
     *   @param selectors CSS selector string of element to 
     *   search for.
     *   @param [container] CSS selector string, 
     *   p5.Element, or HTMLElement to search within.
    ...
    /**
     *   myElement.changed() sets a function to call when 
     *   the value of the p5.Element object changes. 
     *   Calling myElement.changed(false) disables the 
     *   function.
    ...
    /**
     *   myElement.input() sets a function to call when 
     *   input is detected within the p5.Element object. 
     *   It's often used to with text inputs and sliders. 
     *   Calling myElement.input(false) disables the 
     *   function.
    ...
    /**
     *   Creates a <div></div> element. It's commonly used 
     *   as a container for other elements. The parameter 
     *   html is optional. It accepts a string that sets 
     *   the inner HTML of the new <div></div>.
     ...
     *   paragraph-length text. The parameter html is 
     *   optional. It accepts a string that sets the inner 
     *   HTML of the new <p></p>.
    ...
    /**
     *   Creates a <span></span> element. It's commonly 
     *   used as a container for inline elements. For 
     *   example, a <span></span> can hold part of a 
     *   sentence that's a different style. The parameter 
     *   html is optional. It accepts a string that sets 
     *   the inner HTML of the new <span></span>.
     ...
     *   from a given range. The first two parameters, min 
     *   and max, are numbers that set the slider's minimum 
     *   and maximum. 
     * 
     ...
     *   true if it the box is checked and false otherwise. 
     ...
     *   a sketch. The first parameter, callback, is a 
     *   function that's called when the file loads. The 
     *   callback function should have one parameter, file, 
     *   that's a p5.File object. 
     * 
     ...
     *   playback. Returns a new p5.MediaElement object. 
     *   Videos are shown by default. They can be hidden by 
     *   calling video.hide() and drawn to the canvas using 
     *   image(). 
     * 
     ...
     *   Returns a new p5.Element object. Videos are shown 
     *   by default. They can be hidden by calling 
     *   capture.hide() and drawn to the canvas using 
     *   image(). 
     * 
     ...
     *   @return new p5.Element object.
     */
    function createCapture(type?: string|p5.TYPE|object, flipped?: object, callback?: (...args: any[]) => any): p5.Element
     ...
     *   default is CENTER
     */
    function image(img: p5.Image|p5.Element|p5.Framebuffer, dx: number, dy: number, dWidth: number, dHeight: number, sx: number, sy: number, sWidth?: number, sHeight?: number, fit?: p5.IMAGE_FIT, xAlign?: p5.X_ALIGN, yAlign?: p5.Y_ALIGN): void
     ...
     *   is called. Spacing is measured in pixels. Calling 
     *   textLeading() without an argument returns the 
     *   current spacing.
    ...
    /**
     *   Sets the font size when text() is called. Font 
     *   size is measured in pixels. Calling textSize() 
     *   without an arugment returns the current size.
     *   @param size size of the letters in units of pixels
     *   @chainable
     */
     ...
     *   called. textStyle() accepts the following values: 
     *   NORMAL, ITALIC, BOLD or BOLDITALIC. textStyle() 
     *   may be overridden by CSS styling. This function 
     *   doesn't affect fonts loaded with loadFont().
    ...
    /**
     *   Returns the maximum width of a string of text 
     ...
     *   called. textWrap() accepts the following values: 
     ...
     */
    function loadFont(path: string, successCallback?: (...args: any[]) => any, failureCallback?: (...args: any[]) => any): p5.Font
     ...
     *   @chainable
     */
    function text(str: string|object|any[]|number|boolean, x: number, y: number, maxWidth?: number, maxHeight?: number): p5
     ...
     *   @param geometry The geometry whose resources 
     *   should be freed
     */
    ...
    // TODO: Fix imageLight() errors in src/webgl/light.js, line 497:
    //
    ...
    /**
     *   The panorama(img) method transforms images 
     *   containing 360-degree content, such as maps or 
     *   HDRIs, into immersive 3D backgrounds that surround 
     *   your scene. This is similar to calling 
     *   background(color); call panorama(img) before 
     *   drawing your scene to create a 360-degree 
     *   background from your image. It operates on the 
     *   concept of sphere mapping, where the image is 
     *   mapped onto an infinitely large sphere based on 
     *   the angles of the camera. To enable 360-degree 
     *   viewing, either use orbitControl() or try changing 
     *   the orientation of the camera to see different 
     *   parts of the background.
     *   @param img A 360-degree image to use as a 
     *   background panorama
     */
     ...
     *   Options can include: 
     * 
     *   - path: Specifies the location or path of the 3D 
     *   model file for loading.
     ...
     *   is used: camera(0, 0, 800, 0, 0, 0, 0, 1, 0)
     ...
     * 
     *   If no parameters are given, the default values are 
     *   used as: 
     * 
     *   - fov : The default field of view for the camera 
     *   is such that the full height of renderer is 
     *   visible when it is positioned at a default 
     *   distance of 800 units from the camera.
     *   - aspect : The default aspect ratio is the ratio 
     *   of renderer's width to renderer's height.
     *   - near : The default value for the near clipping 
     *   plane is 80, which is 0.1 times the default 
     *   distance from the camera to its subject.
     *   - far : The default value for the far clipping 
     *   plane is 8000, which is 10 times the default 
     *   distance from the camera to its subject. 
     * 
     *   If you prefer a fixed field of view, follow these 
     *   steps: 
     * 
     *   1. Choose your desired field of view angle (fovy). 
     *   This is how wide the camera can see.
     *   2. To ensure that you can see the entire width 
     *   across horizontally and height across vertically, 
     *   place the camera a distance of (height / 2) / 
     *   tan(fovy / 2) back from its subject.
     *   3. Call perspective with the chosen field of view, 
     *   canvas aspect ratio, and near/far values: 
     *   perspective(fovy, width / height, cameraDistance / 
     *   10, cameraDistance * 10);
     ...
     *   height/2, 0, max(width, height) + 800).
     ...
     */
    function loadSound(path: string|any[], successCallback?: (...args: any[]) => any, errorCallback?: (...args: any[]) => any, whileLoading?: (...args: any[]) => any): SoundFile
